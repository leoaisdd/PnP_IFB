function x_out= nuclear_l21(Y,lambda)
...
%                   Y: Observed signal
%             lambda: tuning paramter for nuclear norm term 
%             lambda2: tuning parameter for l21 term
%             M,N     :Dimensions of the signal MXN
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Problem Statement:
%    lambda||x||* +lambda2||Dx||2,1 subjected to y=Ax
...
% of 1/2 ||y-Ax||^2 +lambda||W||* +lambda2||DZ||2,1 +eta1/2||W-X-B1||^2
% +eta2/2||Z-X-B2||^2
...
%%%%% Algorithm steps %%%%%%%%%
 for iteration=1:iter_count
     ...
     [W,s_thld]=nuclear_norm(W,sa2,M,N,lambda,eta1);
     %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
     ...
     objective_func(iteration)=norm(y_vec-A*X)+lambda*s_thld+lambda2*sum(abs(1./l2_row));
